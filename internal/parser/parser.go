...
func (p *parser) peekWithRegex() (token.Pos, token.Token, string) {
	if p.buffered() {
		if len(p.bts) > 1 {
			// Need to figure out how to put back more than one token to the
			// input stream
			panic("peekWithRegex has more than one buffered token")
		}
...
func (p *parser) peekPropertyList() bool {
	if _, tok, _ := p.peek(); tok != token.LPAREN {
		return false
	}

	bts := p.peekN(2)
	if bts[1].tok == token.RPAREN {
		// looks like an empty property list
		return true
	}

	if bts[1].tok != token.STRING && bts[1].tok != token.IDENT {
		return false
	}

	bts = p.peekN(3)
	return bts[2].tok == token.COLON
}
